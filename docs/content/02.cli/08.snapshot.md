...
3. **构建 lock 对象**
   - `source` 会优先读取 repo 当前 Git remote URL。
...
4. **计算校验和**
   ...
   - 该校验和用于 `restore` 时检测 lock 文件是否被意外修改。

5. **写入文件**
   - 如果某个 repo 能解析出真实 Git remote URL，且 `repostack.yaml` 中的 `source` 仍是旧的占位路径值，会先回写 `repostack.yaml`。
...
- `sync` 命令在同步到配置分支最新提交后也会自动重新生成 snapshot。
- `restore` 命令在恢复到 lock revision 后也会自动重新生成 snapshot。
