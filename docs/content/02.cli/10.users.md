...
3. **TTY 环境**：进入交互式菜单，提供以下选项：
   - **List users**：列出可用用户
   - **Switch user**：选择并切换到指定用户
   - **Unset user**：删除 `.repostackrc`
4. **非 TTY 环境**：输出当前用户状态以及可用用户列表，并显示后续可用命令。
5. 如果没有定义任何用户，打印黄色警告并提供手动编辑 `repostack.yaml` 的示例模板。
...
- 后续所有需要用户感知的命令（如 `use`、`remove`、`sync`、`restore` 等）都会读取此文件并合并该用户的配置覆盖。
...
| `.repostackrc` | 读取 / 写入 / 删除 | `su`、`rm`、无子命令模式 |
