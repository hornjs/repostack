...
| `concurrency` | number | `4` | `run` 和 `pull` 命令的默认并发数 |
...
- `sync` 时会切换到这个分支，并尝试 fast-forward 到 upstream 最新提交
- `restore` 时，如果 lock 文件有记录，会切换到 lock 中的 revision
...
命名脚本，通过 `repostack run <script>` 执行。每个脚本可以指定要运行的 shell 命令，以及可选的默认 repo 范围（可被 CLI 参数覆盖）：
...
scripts:
  ...
  # 指定 views：切进每个匹配 repo 执行（多个 views 取并集）
  build:
    command: pnpm build
    views: [runtime]
...
| `repos` | string[] | - | 目标 repo 名称列表 |
| `views` | string[] | - | 目标视图名称列表（多个取并集） |
| `tags` | string[] | - | 目标标签列表（匹配所有指定标签） |

未指定任何范围时，脚本在 stack root 执行一次；指定了范围时，在每个匹配 repo 的目录中执行。
...
| **使用方式** | 在 `scripts` 中用 `tags` 字段 | 在 `scripts` 中用 `views` 字段 |
