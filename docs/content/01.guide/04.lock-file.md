...
| `repos.<name>.source` | 优先记录 Git remote URL；若无 remote，则回退到配置中的 `source` |
...
- `repostack sync` - 同步到配置分支最新提交后重新记录
- `repostack restore` - 恢复到 lock revision 后重新记录
...
**验证时机**：
- `restore` 命令读取 lock 文件时会重新计算并比对
- `pull` / `sync` 为缺失 repo 回退 clone source 而读取 lock 文件时也会进行校验
- 不匹配则报错，防止文件被意外修改
...
- **配合 `restore` 使用**：切换 stack root 历史后运行 `restore`
- **配合 `sync` 使用**：日常开发时同步到配置分支最新提交
...
A: Revision 会显示 `(no commits)`，`restore` 时不会尝试 checkout。

**Q: 可以删除 lock 文件吗？**

A: 可以，但会丢失精确的版本锁定。下次 `restore` 时 repo 会停留在当前 HEAD，而不是某个特定的历史版本。
