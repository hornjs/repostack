...
3. **路径解析与检查**
   ...
   - 如果路径**不存在**：
     - `--yes` 模式：静默创建目录。
     - 非 `--yes` 模式：提示 `Path "..." does not exist. Create it? (y/N)`，默认 **否**；拒绝则中断命令。

4. **Git 仓库检查**
   - 如果目标路径**不是** Git 仓库：
     - `--yes` 模式：自动执行 `git init`。
     - 非 `--yes` 模式：提示 `"..." is not a git repository. Initialize it? (Y/n)`，默认 **是**（直接回车即同意）；拒绝则中断命令。

5. **确认添加到 stack**
   - 非 `--yes` 模式：提示 `Add "..." to repostack? (Y/n)`，默认 **是**；拒绝则中断命令。
