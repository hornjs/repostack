import { join } from "node:path";
import { confirm, isCancel } from "@clack/prompts";
import type { RepostackConfig, RepostackLock } from "../shared/types";
import { checkoutRevision, fetchRepo, isDirty } from "../shared/git";
import { loadLock } from "../shared/lock";
import type { Logger } from "logtra";
import { pull } from "./pull";
import { snapshot } from "./snapshot";

type RestoreOptions = {
  root: string;
  config: RepostackConfig;
  logger?: Logger;
  concurrency?: number;
  yes?: boolean;
};

export async function restore({
  root,
  config,
  logger,
  ...options
}: RestoreOptions): Promise<RepostackLock> {
  logger?.debug(`restore: root=${root} repos=${config.repos.length}`);

  await pull({ root, config, logger, ...options });
  const lock = await loadLock(root, logger);
  logger?.debug(`restore: lock file ${lock ? "found" : "not found"}`);

  for (const repo of config.repos) {
    const cwd = join(root, repo.path);
    logger?.debug(`restore: fetching ${repo.name}`);
    await fetchRepo(cwd);
    const pinned = lock?.repos[repo.name]?.revision;
    if (pinned && pinned !== "(no commits)") {
      logger?.debug(`restore: checking out ${repo.name} @ ${pinned.slice(0, 12)}`);

      const dirty = await isDirty(cwd);
      if (dirty) {
        if (options.yes) {
          logger?.debug(`restore: ${repo.name} has uncommitted changes, proceeding because --yes is set`);
        } else {
          const answer = await confirm({
            message: `Repo "${repo.name}" has uncommitted changes. Checkout ${pinned.slice(0, 12)} anyway?`,
            initialValue: false,
          });
          if (isCancel(answer) || !answer) {
            throw new Error(`Aborted: user declined to checkout ${repo.name}`);
          }
        }
      }

      await checkoutRevision(cwd, pinned);
    }
  }

  return snapshot({ root, config, logger });
}
